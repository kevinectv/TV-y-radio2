    ...
    suspend fun enrichCatalogItem(item: CatalogItem, rawApiKey: String? = null): CatalogItem = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (tmdbId.isNullOrEmpty()) {
                val searchBody = BackendApi.getInstance().search(item.title)
                val root = JSONObject(searchBody)
            ...
            val detailBody = if (isTv) BackendApi.getInstance().getTv(tmdbId) else BackendApi.getInstance().getMovie(tmdbId)
            val root = JSONObject(detailBody)
            ...
            val imagesBody = if (isTv) BackendApi.getInstance().getTvImages(tmdbId) else BackendApi.getInstance().getMovieImages(tmdbId)
            val jsonResponse = JSONObject(imagesBody)
            ...
            val videosBody = if (isTv) BackendApi.getInstance().getTvVideos(tmdbId) else BackendApi.getInstance().getMovieVideos(tmdbId)
            val videosJson = JSONObject(videosBody)
            val results = videosJson.optJSONArray("results")
            if (results != null && results.length() > 0) {
                var ytKey = ""
            ...
            val creditsBody = if (isTv) BackendApi.getInstance().getTvCredits(tmdbId) else BackendApi.getInstance().getMovieCredits(tmdbId)
            val creditsRoot = JSONObject(creditsBody)
            
            // Parse Cast members
            val castArray = creditsRoot.optJSONArray("cast")
            ...
            val crewArray = creditsRoot.optJSONArray("crew")
